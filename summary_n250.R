...
#---------------------------------------------------------------------------------------------------------
## 1. log-normal hub model
#---------------------------------------------------------------------------------------------------------
set.seed(1000)
dataHubCov <- generateSingleDataCell(n = 250, modelCov = 1, p = 500, nRep = 100)
dataCell <- dataHubCov$dataCellN
mat.rho <- as.vector(dataHubCov$sigma)
remove(dataHubCov)
output.file <- "normal_hub_n250_p500.txt"
## COAT
...
#---------------------------------------------------------------------------------------------------------
## 2. log-normal block model
#---------------------------------------------------------------------------------------------------------
set.seed(1000)
dataBlockCov <- generateSingleDataCell(n = 250, modelCov = 2, p = 500, nRep = 100)
dataCell <- dataBlockCov$dataCellN
mat.rho <- as.vector(dataBlockCov$sigma)
remove(dataBlockCov)
output.file <- "normal_block_n250_p500.txt"
## COAT
res <- summaryCorrEst(dataCell)
## metrics summary
t <- dist.auc.roc(dataCell = dataCell, mat.coat.s = res$mat.coat.s, mat.ora.s = res$mat.ora.s,
...
#---------------------------------------------------------------------------------------------------------
## 3. log-normal sparse model
#---------------------------------------------------------------------------------------------------------
set.seed(1000)
dataSparseCov <- generateSingleDataCell(n = 250, modelCov = 3, p = 500, nRep = 100)
dataCell <- dataSparseCov$dataCellN
mat.rho <- as.vector(dataSparseCov$sigma)
remove(dataSparseCov)
output.file <- "normal_sparse_n250_p500.txt"
## COAT
res <- summaryCorrEst(dataCell)
## metrics summary
t <- dist.auc.roc(dataCell = dataCell, mat.coat.s = res$mat.coat.s, mat.ora.s = res$mat.ora.s,
...
mat.rho <- as.vector(dataHubCov$sigma)
remove(dataHubCov)
output.file <- "gamma_hub_n250_p500.txt"
## COAT
res <- summaryCorrEst(dataCell)
## metrics summary
t <- dist.auc.roc(dataCell = dataCell, mat.coat.s = res$mat.coat.s, mat.ora.s = res$mat.ora.s,
...
#---------------------------------------------------------------------------------------------------------
## 5. log-Gamma block model
#---------------------------------------------------------------------------------------------------------
set.seed(1000)
dataBlockCov <- generateSingleDataCell(n = 250, modelCov = 2, p = 500, nRep = 100)
dataCell <- dataBlockCov$dataCellG
mat.rho <- as.vector(dataBlockCov$sigma)
remove(dataBlockCov)
output.file <- "gamma_block_n250_p500.txt"
## COAT
res <- summaryCorrEst(dataCell)
## metrics summary
t <- dist.auc.roc(dataCell = dataCell, mat.coat.s = res$mat.coat.s, mat.ora.s = res$mat.ora.s,
...
#---------------------------------------------------------------------------------------------------------
## 6. log-Gamma sparse model
#---------------------------------------------------------------------------------------------------------
set.seed(1000)
dataSparseCov <- generateSingleDataCell(n = 250, modelCov = 3, p = 500, nRep = 100)
dataCell <- dataSparseCov$dataCellG
mat.rho <- as.vector(dataSparseCov$sigma)
remove(dataSparseCov)
output.file <- "gamma_sparse_n250_p500.txt"
## COAT
res <- summaryCorrEst(dataCell)
## metrics summary
t <- dist.auc.roc(dataCell = dataCell, mat.coat.s = res$mat.coat.s, mat.ora.s = res$mat.ora.s,
