...
summaryCorrEst <- function(dataCell){
  ...
  # mat.coat.h <- matrix(0, p*p, nRep)
  # mat.ora.h <- matrix(0, p*p, nRep)
  mat.ccl <- matrix(0, p*p, nRep)
  mat.reb <- matrix(0, p*p, nRep)
  
  time.coat.s <- vector(length = nRep)
  time.ora.s <- vector(length = nRep)
  # time.coat.h <- vector(length = nRep)
  # time.ora.h <- vector(length = nRep)
  ...
  for (i in 1:nRep){
    ...
    # res.coat.h <- coat(X, soft = 0)
    # corr.coat.h <- res.coat.h$corr
    # mat.coat.h[,i] <- as.vector(corr.coat.h)
    # time.coat.h[i] <- res.coat.h$time
    ...
    # startTime <- proc.time() 
    # res.ora.h <- adaptThresoldCov(logW, soft = 0)
    # exeTimeClass <- proc.time() - startTime
    # res.ora.h.time <- as.numeric(exeTimeClass[3])    
    # corr.ora.h <- res.ora.h$corr
    # mat.ora.h[,i] <- as.vector(corr.ora.h)
    # time.ora.h[i] <- res.ora.h.time
    
    #    res.spa <- SparCC.frac(X)
    #    corr.spa <- res.spa$cor.w
    #    res.spa <- sparcc(X, countStatus = FALSE)
    #    corr.spa <- res.spa$CORR
    #    mat.spa[,i] <- as.vector(corr.spa)
    ...
    #    res.reb <- rebacca(t(X), nbootstrap = 1)
...
dist.auc.roc <- function(dataCell, mat.coat.s, mat.ora.s,
  ...
  # # COAT with hard thresholding
  # pred.coat.h <- prediction(abs(mat.coat.h), abs(mat.rho) > eps + 0)
  # d1.coat.h <- calL1NormLoss(mat.coat.h, mat.rho)
  # df.coat.h <- sqrt(colSums(dmat.coat.h^2))
  # d2.coat.h <- calSpectNormLoss(mat.coat.h, mat.rho)
  # auc.coat.h <- unlist(performance(pred.coat.h, measure = "auc")@y.values)
  # supp.coat.h <- calMatTprFpr(mat.coat.h, mat.rho)
  # tpr.coat.h <- supp.coat.h$tpr
  # fpr.coat.h <- supp.coat.h$fpr
  # d1.df.auc.coat.h <- cbind(d1.coat.h, df.coat.h, d2.coat.h, auc.coat.h, tpr.coat.h, fpr.coat.h, time.coat.h)
  # res.coat.h <- apply(d1.df.auc.coat.h, 2, function(x) {c(mean(x), sd(x))})
  # print("COAT, hard thresholding")
  # print(res.coat.h)
  ...
  # # Oracle with hard thresholding
  # pred.ora.h <- prediction(abs(mat.ora.h), abs(mat.rho) > eps + 0)
  # d1.ora.h <- calL1NormLoss(mat.ora.h, mat.rho)
  # df.ora.h <- sqrt(colSums(dmat.ora.h^2))
  # d2.ora.h <- calSpectNormLoss(mat.ora.h, mat.rho)
  # auc.ora.h <- unlist(performance(pred.ora.h, measure = "auc")@y.values)
  # supp.ora.h <- calMatTprFpr(mat.ora.h, mat.rho)
  # tpr.ora.h <- supp.ora.h$tpr
  # fpr.ora.h <- supp.ora.h$fpr
  # d1.df.auc.ora.h <- cbind(d1.ora.h, df.ora.h, d2.ora.h, auc.ora.h, tpr.ora.h, fpr.ora.h, time.ora.h)
  # res.ora.h <- apply(d1.df.auc.ora.h, 2, function(x) {c(mean(x), sd(x))})
  # print("Oracle, hard thresholding")
  # print(res.ora.h)
  ...
  postscript(file="plot/hub-modify/hub-Gamma-p200.eps", family="Times")
...
dataHubCov <- generateDataCell(n = 200, modelCov = 1, nRep = 100)
# dataBlockCov <- generateDataCell(n = 200, modelCov = 2, nRep = 100)
# dataSparseCov <- generateDataCell(n = 200, modelCov = 3, nRep = 100)

dataCell <- dataHubCov$dataCellG3
# dataCell <- dataBlockCov$dataCellG1
# dataCell <- dataSparseCov$dataCellG3

mat.rho <- as.vector(dataHubCov$sigma3)
# mat.rho <- as.vector(dataBlockCov$sigma1)
# mat.rho <- as.vector(dataSparseCov$sigma3)
...
t <- dist.auc.roc(dataCell = dataCell, mat.coat.s = res$mat.coat.s, mat.ora.s = res$mat.ora.s,
                  mat.ccl = res$mat.ccl, mat.reb = res$mat.reb, mat.rho = mat.rho,
                  time.coat.s = res$time.coat.s, time.ora.s = res$time.ora.s,
                  time.ccl = res$time.ccl, time.reb = res$time.reb)

# t <- dist.auc.roc(dataCell = dataCell, mat.coat.s = res$mat.coat.s, mat.ora.s = res$mat.ora.s,
#                    mat.coat.h = res$mat.coat.h, mat.ora.h = res$mat.ora.h,
#                    mat.ccl = res$mat.ccl, mat.reb = res$mat.reb, mat.rho = mat.rho,
#                    time.coat.s = res$time.coat.s, time.ora.s = res$time.ora.s,
#                    time.coat.h = res$time.coat.h, time.ora.h = res$time.ora.h,
#                    time.ccl = res$time.ccl, time.reb = res$time.reb)

#t <- dist.auc.roc.coat.ora(dataCell = dataCell, mat.coat.s = res$mat.coat.s, mat.ora.s = res$mat.ora.s, 
#                  mat.coat.h = res$mat.coat.h, mat.ora.h = res$mat.ora.h, mat.rho = mat.rho,
#                  time.coat.s = res$time.coat.s, time.ora.s = res$time.ora.s, 
#                  time.coat.h = res$time.coat.h, time.ora.h = res$time.ora.h)

save.image("plot/hub-modify/hub-Gamma-p200-modify.RData")
